...
folder_name = f"training_{CONFIG['fbp_angles']}_{CONFIG['fbp_det_spacing']}_{CONFIG['fbp_cutoff']}_unet3"
...
class MedicalImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Normalizzazione Min-Max per scalare l'FBP tra 0 e 1
        fbp_img = np.clip(fbp_img, 0, None) # Rimuovi valori negativi (artefatti comuni in FBP)
        if fbp_img.max() > 0:
            fbp_img = fbp_img / fbp_img.max()
...
def train(config):
    ...
    criterion = CombinedLoss(alpha=1.0, beta=1.0, gamma=0.1).to(config["device"])
